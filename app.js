...
const activePadOscillators = {}; // Almacena los osciladores activos del Pad
...
function noteOn(note, velocity, fromMouse = false) {
    ...
    // Activar Pad si está encendido
    if (padEnabled) {
        activePadOscillators[note] = playPadNote(note, padPresetSelect.value);
    }
...
function stopPadInstance(note) {
    if (activePadOscillators[note]) {
        ...
        const now = audioCtx.currentTime;
        const releaseTime = 0.8; // Liberación suave para el pad
...
function playPadNote(midiNote, padType) {
    ...
    // Escala de volumen del Pad según la perilla (niveles 1 al 5)
    ...
    if (padType === 'warm_pad') {
        ...
        osc2.type = 'triangle';
        osc1.frequency.value = freq;
        osc2.frequency.value = freq * 1.005; // Ligeramente desafinado para grosor analógico
        ...
        osc2.type = 'sawtooth';
        osc1.frequency.value = freq;
        osc2.frequency.value = freq * 2; // Octava superior
    ...
    // Envolvente de ataque lento típico de un Pad de sintetizador
